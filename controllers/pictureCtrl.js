...
module.exports = {
    ...
    sendPicture: function (req, res) {
        ...
        if( !isNaN(place) || place=='undefined' || !regexPlace.test(place) || place.length < 2 || place.length > 60) {
            fs.unlink(__dirname + '/../uploads/' + filename, function(error) {
                if (error) {
                    throw error;
                }
                // console.log('Deleted cause place '+ filename);
                return res.json({status:"failure", message: 'Erreur sur le champ Lieu'});
            });
        } else if ( isNaN(year) || year=='undefined' || year.length != 4 || year > today.getFullYear() || year < 1900 ) {
             fs.unlink(__dirname + '/../uploads/' + filename, function(error) {
                if (error) {
                    throw error;
                }
                // console.log('Deleted cause year '+ filename);
              ...
              // console.log(rank);
                if (rank >= 32) {
                    fs.unlink(__dirname + '/../uploads/' + filename, function(error) {
                        ...
                        // console.log('Deleted cause complete '+ filename);
                        return res.json({status:"failure", message: 'Vous ne pouvez pas ajouter la photo car cette galerie est complète.'});
                    });
                }
                else {
                    let next;
                    ...
                    dimensions.width > dimensions.height ? w = 200 : w = 150;
                    ...
                    return countPicturesByColor(newColor).then(result=>{
                        ...
                        if (result >= 32) {
                            ...
                            return models.photo.update({place: newPlace, year: newYear, month: newMonth, color_name: newColor}, {where: {id: unikId  }}).then(result => {
                                ...
                                return res.json({status:"success", message: "Vos modifications sont enregistrées"});
                            }).catch(error =>{
                                return res.json({status:"failure", message:error.toString()});
                            });
                        }
