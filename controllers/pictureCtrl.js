  ...
  //    console.log(res);
    ...
    sendPicture: function (req, res) {
        ...
        models.photo.max('rank', { where: { color_name: color }}).then(function(rank) {
          // console.log(rank);
          if (rank >= 32) {
            return res.json({status:"failure", message: 'Vous ne pouvez pas ajouter cette photo car la gallerie '+color+' est complète.'});
          }
          // else if (isNaN(rank)) {
          //   let next = rank+1;
          //   return res.json({status:"success", message: 'C\'est la 1ère photo pour la couleur '+color+', Bravo !'});
          // }
          else {
            let next;
            if (isNaN(rank)) {
            ...
            // si le champ n'est pas rempli, il est undefined -> trouver façon de tester le champ qd il est undefined
            if(year=='undefined' || year.length != 4){
                return res.json({status:"failure", message: 'Erreur sur le champ Year'});
            }
            ...
            models.photo.create({file_name: filename, thumb_name: thumbname, place: place, year: year, month: month, name: pic_name, size: pic_size, orientation: orientation, color_name: color, rank: next}).then(result=>{
                if (next == 32) {
                    return res.json({status:"success", message: "Vous avez ajouté la photo " + pic_name + " à votre galerie "+color+". Elle est maintenant complète."});
                } else {
                    return res.json({status:"success", message: "Vous avez ajouté la photo " + pic_name + " à votre galerie "+color});    
                }
